...
// Check Cover
if (!fs.existsSync(path.join(PHOTO_DIR, travelData.coverImage))) {
    console.warn(`⚠️  COVER MISSING: ${travelData.coverImage}`);
    missingCount++;
}

// Check Day Photos
// Updated Audit for Objects
...
if (missingCount === 0) {
    ...
    console.log(`❗ AUDIT COMPLETE: ${missingCount} assets missing. Continuing build with placeholders...`);
}

// 3. IMAGE HELPER (The Fix)
// NEW: Helper for Image with Caption
function insertImageWithCaption(imgObj) {
    const imgName = typeof imgObj === 'string' ? imgObj : imgObj.url;
    const captionText = typeof imgObj === 'object' ? imgObj.caption : "";

    const imgPath = path.join(PHOTO_DIR, imgName);

    if (!fs.existsSync(imgPath)) {
        return [new Paragraph({ children: [new TextRun({ text: `[MISSING: ${imgName}]`, color: "FF0000" })] })];
    }
...
const doc = new Document({
    ...
    sections: [{
        ...
        children: [
            ...
            insertImage(travelData.coverImage, true),
            ...
            // 5. Updated Day Loop 
						// UPDATE: Inside the Day Loop flatMap
						...travelData.days.flatMap(day => {
						    ...
						    // Handle the new Multi-Image + Caption logic
						    if (day.images && Array.isArray(day.images)) {
						        day.images.forEach(imgObj => {
